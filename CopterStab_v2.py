...
def level_deviation(history, cnt_outliers=0, altitude = 1):
    ...
    #print(f"{lev_dev=}, {correction=}, {max_dev=}")
    return lev_dev
...
def main():
    ...
    while True:
        ...
        if (len(prev_kp) < min_kp or len(prev_kp) > max_kp) and contrast > 0.01 and contrast < 0.1:
            ...
            print(len(curr_kp), contrast)
        else:
            print("break")
            break

    first_center = np.array([[[img_w / 2, img_h / 2]]], dtype=np.float32)
    ...
    while True:
        ...
        if center and not is_outlier:
            cv2.circle(display, (int(img_w/2+dx), int(img_h/2-dy)), 2, (0, 0, 255), 2)
            ...
            center_x, center_y = int(img_w / 2), int(img_h / 2)
            ...
            print(f"{text} | {contrast=:.3f}, {threshold_matcher=:.2f}, kp1={prev_len_kp}, kp2={len(curr_kp)}, mat={len(matches)}")
        else:
            frame_errors_cnt += 1
            print(f"\nТрекинг потерян: {contrast=:.3f}, {threshold_matcher=:.2f}, kp1={prev_len_kp}, kp2={len(curr_kp)}, mat={len(matches)}\n")
